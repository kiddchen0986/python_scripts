"""Run the dead_pixels test by running both checkerboard and inverted checkerboard"""
import argparse
import os
...
from wrapper.run_native_test import run_checkerboard

libc = None


def run_dead_pixels(check_image,
                    inv_check_image,
                    product_type,
                    hwid,
                    test_seq_pack):
    """Run the dead pixels test; checkerboard and inverted checkerboard

    Run the native tests sequentially; checkerboard and inverted checkerboard. Add them to the dict
    ...
    if "test_sequence" not in test_seq_pack:
        ...
        test_seq_pack["test_sequence"] = "dead_pixels"
    ...
    test_seq_pack["test_packages"].append(
        run_checkerboard(libc, check_image, product_type, hwid))

    # Run the inverted checkerboard test
    test_seq_pack["test_packages"].append(
        run_checkerboard(libc,
                         inv_check_image,
                         product_type,
                         hwid,
...
def run(dir_path, product_type_=None, hwid_=None):
    ...
    for id_, vals in logs.items():
        ...
        seq_packs.append(run_dead_pixels(checker_image,
                                         checker_inv_image,
                                         product_type,
                                         hwid,
                                         {"test_id": id_}))
    ...
    csv.write_top_of_file(analyze_container, fp, "Dead pixels test", "", "", "Directory: {}".format(dir_path))
...
if __name__ == "__main__":
    ...
    # Parse user-input
    parser = argparse.ArgumentParser(description="Run the dead_pixels test on a directory of logs and images")
