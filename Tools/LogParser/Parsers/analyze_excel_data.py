...
class analyze_excel:
    ...
    def get_valid_data_by_key(self, key):
        rb = xlrd.open_workbook(self.file)
        sheet = rb.sheet_by_name('Sheet1')

        for i in range(0, sheet.ncols):
            if str(sheet.cell_value(0, i)) == str(key):
                return self.get_valid_data(i)
        return None

    def get_valid_datas_by_key(self, key, keep_max=True):
        df = pd.read_excel(self.file)
        df.dropna(subset=['sensorid'], inplace=True)
        df.sort_values(by=['sensorid', key], ascending=not keep_max, inplace=True)
